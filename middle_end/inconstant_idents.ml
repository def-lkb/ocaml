...
module Inconstants (P:Param) (Backend:Backend_intf.S) = struct
  ...
  *)
  ...
  and mark_named ~toplevel curr (named : Flambda.named) =
    ...
    *)
    | Prim (Lambda.Pmakeblock (_tag, Asttypes.Immutable, _value_kind, _tagl), args,
            _dbg) ->
      mark_vars args curr
...
*)
    | Prim (Pmakearray (Pfloatarray, Immutable, _tagl), args, _) ->
      mark_vars args curr
    | Prim (Pmakearray (Pfloatarray, Mutable, _tagl), args, _) ->
