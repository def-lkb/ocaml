...
(* The [lazy]s are to ensure that we don't create [Ident.t]s at toplevel
   ...
   arch.ml.) *)
let spacetime_node = ref (lazy (Cmm.Cvar (Ident.create_dummy "dummy")))
let spacetime_node_ident = ref (lazy (Ident.create_dummy "dummy"))
...
let code_for_function_prologue ~function_name ~node_hole =
  let node = Ident.create_dummy "node" in
  let new_node = Ident.create_dummy "new_node" in
  let must_allocate_node = Ident.create_dummy "must_allocate_node" in
  let is_new_node = Ident.create_dummy "is_new_node" in
  ...
  let initialize_direct_tail_call_points_and_return_node =
    let new_node_encoded = Ident.create_dummy "new_node_encoded" in
    ...
    | _ ->
      Clet (new_node_encoded,
        ...
        body)
  in
  let pc = Ident.create_dummy "pc" in
...
let code_for_blockheader ~value's_header ~node ~dbg =
  ...
  let existing_profinfo = Ident.create_dummy "existing_profinfo" in
  let existing_count = Ident.create_dummy "existing_count" in
  let profinfo = Ident.create_dummy "profinfo" in
  let address_of_profinfo = Ident.create_dummy "address_of_profinfo" in
...
let code_for_call ~node ~callee ~is_tail ~label =
  ...
  let place_within_node = Ident.create_dummy "place_within_node" in
  let dbg = Debuginfo.none in
  let open Cmm in
  Clet (place_within_node,
    ...
    | Direct _callee ->
      if Config.enable_call_counts then begin
        let count_addr = Ident.create_dummy "call_count_addr" in
        let count = Ident.create_dummy "call_count" in
        Clet (count_addr,
...
class virtual instruction_selection = object (self)
  ...
  method private instrument_indirect_call ~env ~callee ~is_tail
    ...
    let callee_ident = Ident.create_dummy "callee" in
  ...
  method! emit_fundecl f =
    if Config.spacetime then begin
      disable_instrumentation <- false;
      let node = Ident.create_dummy "spacetime_node" in
