...
let type_format loc fmt =
  ...
  let ty_arrow gty ty = newty (Tarrow (Tarr_simple, instance_def gty, ty, Cok)) in
      ...
      let func texp =
        let e =
          {texp with exp_type = ty_res; exp_desc =
           Texp_apply
             (texp,
              List.rev args @ [Tarr_simple, Some eta_var, Required])}
        in
...
let report_error env ppf = function
  ...
  | Apply_wrong_label (l, ty) ->
      let print_label ppf = function
        ...
        | Tarr_labelled s -> fprintf ppf "with label ~%s" s
      in
